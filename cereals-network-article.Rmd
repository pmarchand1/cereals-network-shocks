...
![Simulation model flow chart. See Table \ref{tab:var_list} for a list of variables. Note that "res. shock" is the residual shock at a given step, and "Block outgoing trade flows" means that trade volume can only decrease along those links. \label{fig:sim_chart}](model-flow-chart.pdf)
...
plot_grid(dCplot_self, dCplot_ext, label_size = 12, labels = c("a", "b"), 
          vjust = 1, ncol = 1, scale = 0.9, align = "v")
```
